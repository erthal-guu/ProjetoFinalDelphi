...
begin
  ...
  begin
    try
      ...
      QueryEntradas.SQL.Add
        ('    SUM(SUM(r.valor_total)) OVER () AS valor_total_geral_receitas');
  ...
  begin
    try
      ...
      QueryCanceladas.SQL.Add('    MAX(r.data_emissao) AS data_ultimo_cancelamento,');
      QueryCanceladas.SQL.Add('    COALESCE(AVG(r.valor_total), 0) AS ticket_medio_cancelado,');
  ...
  begin
    try
      ...
      QueryPendentes.SQL.Add('    COALESCE(AVG(r.valor_total), 0) AS valor_medio_pendente,');
      QueryPendentes.SQL.Add('    COALESCE(SUM(r.valor_total), 0) AS valor_total_pendentes,');
  ...
  begin
    try
      ...
      QueryPendenciasPendentes.SQL.Add('    COUNT(CASE WHEN p.data_vencimento >= CURRENT_DATE THEN 1 END) AS quantidade_a_pagar,');
      ...
      QueryPendenciasPendentes.SQL.Add('ORDER BY f.nome');
